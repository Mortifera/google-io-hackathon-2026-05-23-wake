/**
 * worlds/notion/gen-dossiers.ts — generate Tier-2/Tier-3 dossiers with Gemini
 * Flash (the "mass info" lane; Tier-1 stays human-curated in dossiers.t1.ts).
 *
 * Reads the node list from world.json, asks gemini-3.5-flash to rewrite each
 * tier-2/3 node's first-pass draft into a vivid ~80-110 word dossier, and
 * writes worlds/notion/dossiers.bulk.json. build.ts then overrides tier-2/3
 * dossiers from that file (inline drafts remain the offline fallback).
 *
 * These nodes are archetypes / aggregates, never real named individuals, so no
 * real-person facts or quotes are invented (AGENTS.md §4). Cost is a few cents
 * of live Flash (well within the ~$20 budget). Calls the REST API directly with
 * fetch — no new dependency, no reliance on @wake/llm.
 *
 * Run: pnpm exec tsx worlds/notion/gen-dossiers.ts
 */
...
const targets = world.nodes.filter((n) => n.tier !== 1);

const SYSTEM = `You write concise "dossiers" for nodes in Wake, a multi-agent world simulation. The focal scenario is Microsoft acquiring Notion (with adjacent shocks: Notion guts its free tier, CEO Ivan Zhao steps down, Notion open-sources its core engine).

Each dossier is cached system-prompt material that drives how the node behaves during a cascade. A good dossier captures: the node's voice/behavior, how it filters and reacts to news, its biases and sensitivities, and what would flip its sentiment. For aggregate/count nodes, note that it is a composite with averaged sentiment and low individual signal.
    ...
    contents: [
      {
        ...
        parts: [
          {
            text: `Rewrite and enrich each node's draft into a final dossier (80-110 words). Preserve its meaning and the scenario relevance; make it sharper and more specific. Return one object per node.\n\n${userLines}`,
          },
        ],
      },
    ],
...
for (const [i, batch] of batches.entries()) {
  ...
  await sleep(400); // be gentle on Tier-1 rate limits
}
...
// Rough Flash cost estimate (input ~$0.30/M, output ~$2.50/M as a ballpark).
