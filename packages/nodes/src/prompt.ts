...
export function buildSystemPrompt(input: TickInput): string {
  ...
  return [
    ...
    ``,
    `An event of type "action" from "world" is an ESTABLISHED FACT that has`,
    `already happened — treat it as real and respond to that reality. Do not`,
    `deny it, call it a rumor, or wait for confirmation.`,
    ``,
