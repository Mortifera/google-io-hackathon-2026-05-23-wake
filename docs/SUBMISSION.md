...
| **3f** | **Variant A/B + what the sweep settles on** — "we ran your announcement two ways; the pivotal variable points to 'independent'" | The decision-optimization wedge, phrased instrument-not-advisor | **partial (re-scoped)** — no dedicated A/B "Variant Lab" view shipped; the wedge is expressed via the fan + the pivotal card naming the framing lever in instrument register, and the two-worlds beat is the shipped A/B. The "Variant Lab" mock (`somedesignwork.pen`) is fully designed but unbuilt. |
